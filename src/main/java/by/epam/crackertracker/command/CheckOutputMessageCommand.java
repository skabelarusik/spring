/**
 * command to check output messages in inner post
...
public class CheckOutputMessageCommand implements Command {
    private static final Logger LOGGER = Logger.getRootLogger();
    public static final int MAX_TABLE_MESSAGE = 6;
    private MessageService service;

   public CheckOutputMessageCommand(MessageService service){
    ...
    public String execute(HttpServletRequest request) {
        ...
        String page;
        try{
            messageList = service.checkOutputMessage(login, pageNumber);
            ...
            request.setAttribute(ParameterConstant.ATTRIBUTE_LIST_MESSAGE_OUT, newMessageList);
            page = PageConstant.PATH_PAGE_MESSAGE;
        } catch (TrackerServiceException e){
            LOGGER.error("Command can't check output messages users: " + login, e);
